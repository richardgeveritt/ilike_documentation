...
The specification of a state-space model, which involves a time index $t$ and requires the specification of a transition model, is not described in this section: these details can be found in section \@ref(ssm).

There are two ways of specifying priors: through user-defined functions, or through simple built-in "ilike" functions. For a likelihood, the specification through user-defined functions closely follows the analogous way of specifying a prior. The built-in choices of likelihood can be categorised as follows:

- linear and non-linear Gaussian models, covering the special case of measurements with Gaussian noise;

- simulation-based likelihoods, allowing the use of data models that are specified in the form of a simulator;

- algorithmic likelihoods, where the likelihood is given by an algorithm such as a particle filter.

Recall that every entry in an ilike file has a "function type". This section details every function type that may be used to specify a model: each subsubsection (e.g. 3.1.1) is named after a function type. For user-defined function types we detail the input and output arguments of the function, and give an example of its use. The specification of each function type is described in both C++ and R. Python and Julia function are both called from R - see section \@ref(ilike-files) - so do not require further description). For built-in (ilike) function types, we detail each available built-in function in subsubsubsections (e.g. 3.2.1.1).
...
## Linear-Gaussian model {#linear-gaussian-data-model}
...
## Nonlinear-Gaussian model {#nonlinear-gaussian-data-model}
...
Inputs:
  ...
  4. (optional) boolean: whether to store the output of the model simulations (default is `false`)
...
Inputs:
  ...
  5. (optional) boolean: whether to store the output of the model simulations (default is `false`)
  
  6. (optional) string: the name of the variable that stores the scaling $\sigma_i$ of the data/summaries in the ABC (default is `""` - in this case the scale will be `1` for every dimension)
  
  7. (optional) numeric vector: the scaling of the data/summary in the ABC, which must be of the same dimension as the data/summary This is only used if the scale variable (argument 6) is set.
...
Inputs:
  ...
  1. string: The name of an ilike file (with extension) that contains the specification of a linear-Gaussian state-space model, as described in section \@ref(ssm). A full description of what is required in the file is given in section \@ref(inference-kf).
...
The particle filter is a recursive algorithm that estimates the state of a general state-space model and, in this context, can be used to estimate the marginal likelihood of parameters of the model. The arguments required for this ilike function follow as closely as possible those required for running a PF from the ilike R package as described in section \@ref(inference-pf).

Inputs:

  1. string: The name of an ilike file (with extension) that contains the specification of a general state-space model, as described in section \@ref(ssm). A full description of what is required in the file is given in section \@ref(inference-pf).
...
The ensemble Kalman filter (EnKF) is a recursive algorithm that estimates the state of a general state-space model and, in this context, can be used to estimate the marginal likelihood of parameters of the model. The arguments required for this ilike function follow as closely as possible those required for running an EnKF from the ilike R package as described in section \@ref(inference-enkf).

Inputs:

  1. string: The name of an ilike file (with extension) that contains the specification of a general state-space model, as described in section \@ref(ssm). A full description of what is required in the file is given in section \@ref(inference-enkf).

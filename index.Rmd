...
This installation will take some time, so go and make a cup of tea (or even lunch?) at this point.
...
<!-- * iterative ensemble Kalman inversion ABC (IEnKI-ABC). -->
...
Item 1 defines the data; items 2&3 define a model; items 4&5 define provide all of the elements required to specify a Metropolis algorithm. Together we refer to this as a "recipe". To use the file it must first be compiled into an ilike "recipe" object. To do this we call

```{r,eval=FALSE}
library(ilike)
recipe = compile("toy_model_rwm.ilike")
```
...
We run the Metropolis algorithm on this recipe by calling:
...
The first argument is the `recipe` we compiled in the previous step. `results_name`, specifies the folder where the results will be written. `initial_values` gives the initial values of the chain. `model_parameter_list` gives the value of `p1`, the proposal covariance.
...
library(ggsmc)
plot_scatter(output,x_parameter = "θ",x_dimension=1,y_parameter="θ",y_dimension=2,mcmc = TRUE)
```

![](toy_scatter.png)

We can plot trace plots using, for example, [`ggmcmc`](https://CRAN.R-project.org/package=ggmcmc)

```{r,eval=FALSE}
output = load_mcmc_output("toy_model_rwm", ggmcmc = TRUE)
library(ggmcmc)

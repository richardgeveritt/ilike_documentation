...
* It has a return type (`double`) and argument(s) (`const Parameters &`)^[If the argument, for example, seem mysterious to you, don't worry about this just yet. This documentation should tell you most of what you need to know to get started. If you want to understand more, [this tutorial](https://cplusplus.com/doc/tutorial/functions/) is a reasonable starting point.] that are pre-determined by the ilike package. The key point is that if a function under the `evaluate_log_prior` type does not have this return type and arguments (including the `const` and the `&`), you will receive an error.
